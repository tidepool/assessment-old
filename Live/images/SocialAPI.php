...
function checkIfInSystem($email, $password)
{
    $sql = sprintf("SELECT Password,Stage,ID,Name,Pic,Gender,WorkTypeTitle,FacebookID,LinkedID FROM SocialMediaUsers WHERE Email='%s'",mysql_real_escape_string($email));
    $result = mysql_query($sql);
    $checker = mysql_result($result, 0,  0);
...
function signUpUser($name,$email,$password,$gender)
{
    global $facebook;
    ...
    else if(mysql_affected_rows()==0)
    {
        ...
        $browser = getBrowser();
        $wt = $_COOKIE['WTname'];
        $num = strpos($_COOKIE['ID'],"O");
        $unq = substr($_COOKIE['ID'],0,$num);
        ...
        sendVerifyEmail($email,$name,$verifyLink,$removeLink);
...
function getTidepoolFriends($ID)
{
