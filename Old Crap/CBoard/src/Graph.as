package  
{
	...
	public class Graph 
	{
		...
		public function Graph(p_main:Object,p_page:ComparePage,px:Number,py:Number,pw:Number=800,ph:Number=400) 
		{
			main = p_main;
			page = p_page;
			positionX = px;
		...
		{
			rootNode = new GraphNode(main, this, positionX+width/2, positionY+height/2, nodeData[root],0);
			nodes.push(rootNode);
		...
		public function createNeighbor(rootNode:GraphNode,layer:int):void
		{
			...
			for (var i:int = 0; i < neibData.length; i++ )
			{
				...
				if (layer == 1)
				{
					if (ch==null)
					{
						n = new GraphNode(main, this, rootNode.positionX + 150 * Math.sin(i * 1), rootNode.positionY + 100 * Math.cos(i * 1), neibData[i], layer);
