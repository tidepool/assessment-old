...
$query = "SELECT sub_dimension FROM clouds_scoring WHERE user_id = '".$ID."'";
$result = mysql_query($query) or die('Query failed: ' . mysql_error());
$temp = mysql_fetch_row($result);
$holland = $temp[0];
mysql_free_result($result);

$query = "SELECT sub_dimension FROM frames_model_score_without_space WHERE user_id = '".$ID."'";
$result = mysql_query($query) or die('Query failed: ' . mysql_error());
$temp = mysql_fetch_row($result);
$personality = $temp[0];
mysql_free_result($result);
...
$query = "SELECT title FROM worktypes WHERE worktype_id='$code';";
$result = mysql_query($query) or die('Query failed: ' . mysql_error());
$title = mysql_result($result, 0, 0);
mysql_free_result($result);
setcookie("WTname", $title, time()+7200,"/", ".tidepool.co"); /* Expires in two hours */

$query = "UPDATE delta_users SET worktype='$code' WHERE user_id='$ID';";
$result = mysql_query($query) or die('Query failed: ' . mysql_error());
mysql_free_result($result);
