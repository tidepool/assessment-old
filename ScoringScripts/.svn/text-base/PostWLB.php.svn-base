...
function scoreTravel()
{
    ...
    for($i=0;$i<4;$i++)
    {
        ...
        if($travel[$i]['value'] <= 20)
        {
            $ISOQuery .= ", 0";
        }
        else if($travel[$i]['value'] <= 40)
        {
            $ISOQuery .= ", 1";
        }
        else if($travel[$i]['value'] <= 60)
        {
            $ISOQuery .= ", 2";
        }
        else if($travel[$i]['value'] <= 80)
        {
            $ISOQuery .= ", 3";
        }
        else if($travel[$i]['value'] <= 100)
...
function readInputs()
{
    ...
    Global $pieChng, $slider1Chng, $shivaChng, $familyChng, $slider2Chng, $couchChng, $dreamChng, $trashChng, $clockChng, $netsChng, $checkChng, $briefcaseChng, $officeChng;
    Global $xml_array;
    ...
    foreach($xml_array as $values)
    {
        ...
        {
            $travel['changes'] = array();
            foreach($values->children() as $value)
            {
                if($value->getName() == "pie")
                {
                    $pieChng = $value;
                }
                ...
                else if($value->getName() == "shiva")
                {
                    $shivaChng = $value;
                }
                else if($value->getName() == "travel")
                {
                ...
                else if($value->getName() == "couch")
                {
                    $couchChng = $value;
                }
                else if($value->getName() == "nets")
                {
                    foreach($value->children() as $v)
                    {
                        $netsChng[] = $v;
                ...
                else if($value->getName() == "briefcase")
                {
                    foreach($value->children() as $v)
                    {
                        $briefcaseChng[] = $v;
                ...
                else if($value->getName() == "dream")
                {
                    $dreamChng = $value;
                }
                else if($value->getName() == "trash")
                {
                    $trashChng = $value;
                }
                else if($value->getName() == "clock")
                {
                    $clockChng = $value;
                }
                else if($value->getName() == "check")
                {
                    $checkChng = $value;
                }
            }
...
function uploadISO()
{
    ...
    {
        $query = "UPDATE Timing SET WLB='$date' where ID='$ID';";
    }
    $result = mysql_query($query) or die('Query failed: ' . mysql_error());
    mysql_free_result($result);
...
{
    Global $ID, $link, $pieChng, $slider1Chng, $shivaChng, $travel,  $familyChng, $slider2Chng, $couchChng, $netsChng,  $briefcaseChng, $officeChng, $dreamChng, $trashChng, $clockChng, $checkChng;

    $queryChunk = "'$ID', '$pieChng', '$slider1Chng', '$shivaChng'";
    foreach($travel['changes'] as $change)
    {
        $queryChunk .= ",'$change'";
    }
    $queryChunk .= ",'$familyChng', '$slider2Chng', '$couchChng'";
    foreach($netsChng as $change)
    {
        $queryChunk .= ",$change";
    };
    foreach($briefcaseChng as $change)
    {
        $queryChunk .= ",$change";
    };
    foreach($officeChng as $change)
    {
        $queryChunk .= ",$change";
    }
    $queryChunk .= ",$dreamChng, $trashChng, '$clockChng', '$checkChng'";
    //echo $queryChunk;
    $query = "INSERT INTO WLBChanges VALUES ($queryChunk);";
    $result = mysql_query($query) or die('Query failed: ' . mysql_error());
    //echo $result;
    mysql_free_result($result);
    mysql_close($link);
